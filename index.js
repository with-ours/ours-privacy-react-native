...
export class OursPrivacy {
  ...
  /**
   * Returns a OursPrivacy People object that can be used to set and increment
   * People Analytics properties.
   *
   * @return {People} an instance of People that you can use to update
   *     records in OursPrivacy People Analytics
   */
  getPeople() {
    if (this.people) {
      return this.people;
    } else {
      this.people = new People(this.token, this.oursprivacyImpl);
      return this.people;
    }
  }
  ...
  trackWithGroups(eventName, properties, groups) {
    ...
    this.oursprivacyImpl.trackWithGroups(
      this.token,
      eventName,
      {
        ...Helper.getMetaData(),
  ...
  setGroup(groupKey, groupID) {
    ...
    this.oursprivacyImpl.setGroup(this.token, groupKey, groupID);
  }

  /**
   * Returns a OursPrivacyGroup object that can be used to set and increment
   ...
   * @return an instance of OursPrivacyGroup that you can use to update
   *     records in OursPrivacy Group Analytics
   */
  getGroup(groupKey, groupID) {
    if (this.group) {
      return this.group;
    } else {
      this.group = new OursPrivacyGroup(
        this.token,
        groupKey,
        groupID,
        this.oursprivacyImpl
  ...
  addGroup(groupKey, groupID) {
    ...
    this.oursprivacyImpl.addGroup(this.token, groupKey, groupID);
  }
  ...
  removeGroup(groupKey, groupID) {
    ...
    this.oursprivacyImpl.removeGroup(this.token, groupKey, groupID);
  }
  ...
  deleteGroup(groupKey, groupID) {
    ...
    this.oursprivacyImpl.deleteGroup(this.token, groupKey, groupID);
  }
...
/**
 * Core class for using OursPrivacy People Analytics features.
 *
 * <p>The People object is used to update properties in a user's People Analytics record,
 * and to manage the receipt of push notifications sent via OursPrivacy Engage.
 ...
 */
export class People {
  constructor(token, oursprivacyImpl) {
    if (!StringHelper.isValid(token)) {
      StringHelper.raiseError(PARAMS.TOKEN);
    }
    this.token = token;
    this.oursprivacyImpl = oursprivacyImpl;
  }

  /**
   * Sets a single property with the given name and value for this user.
   * The given name and value will be assigned to the user in OursPrivacy People Analytics,
   * possibly overwriting an existing property with the same name.
   *
   * @param {string} prop The name of the OursPrivacy property. This must be a String, for example "Zip Code"
   * @param {object} to The value of the OursPrivacy property. For "Zip Code", this value might be the String "90210"
   */
  set(prop, to) {
    let properties = {};
    ...
    this.oursprivacyImpl.set(this.token, properties);
  }
  ...
  setOnce(prop, to) {
    ...
    this.oursprivacyImpl.setOnce(this.token, properties);
  }
  ...
  increment(prop, by) {
    ...
    this.oursprivacyImpl.increment(this.token, add);
  }
  ...
  append(name, value) {
    ...
    if (DevicePlatform.iOS === Helper.getDevicePlatform()) {
      this.oursprivacyImpl.append(this.token, appendProp);
    } else {
      this.oursprivacyImpl.append(this.token, name, value);
    }
  }
  ...
  union(name, value) {
    ...
    if (DevicePlatform.iOS === Helper.getDevicePlatform()) {
      this.oursprivacyImpl.union(this.token, {[name]: value});
      this.oursprivacyImpl.union(this.token, {[name]: value});
    } else {
      this.oursprivacyImpl.union(this.token, name, value);
    }
  ...
  remove(name, value) {
    ...
    if (DevicePlatform.iOS === Helper.getDevicePlatform()) {
      this.oursprivacyImpl.remove(this.token, removeProp);
    } else {
      this.oursprivacyImpl.remove(this.token, name, value);
    }
  }
  ...
  unset(name) {
    ...
    this.oursprivacyImpl.unset(this.token, name);
  }
  ...
  trackCharge(charge, properties) {
    ...
    this.oursprivacyImpl.trackCharge(this.token, charge, properties || {});
  }
   ...
   */
  clearCharges() {
    this.oursprivacyImpl.clearCharges(this.token);
  }
   ...
   */
  deleteUser() {
    this.oursprivacyImpl.deleteUser(this.token);
  }
}

/**
 * Core class for using OursPrivacy Group Analytics features.
 *
 * <p>The OursPrivacyGroup object is used to update properties in a group's Group Analytics record.
 */
export class OursPrivacyGroup {
  constructor(token, groupKey, groupID, oursprivacyImpl) {
    if (!StringHelper.isValid(token)) {
      StringHelper.raiseError(PARAMS.TOKEN);
    }
    this.token = token;
   ...
   * The given name and value will be assigned to the user in OursPrivacy Group Analytics,
   * possibly overwriting an existing property with the same name.
   *
   * @param {string} prop The name of the OursPrivacy property. This must be a String, for example "Zip Code"
  ...
  set(prop, to) {
    ...
    this.oursprivacyImpl.groupSetProperties(
      this.token,
      this.groupKey,
      this.groupID,
      properties
  ...
  setOnce(prop, to) {
    ...
    this.oursprivacyImpl.groupSetPropertyOnce(
      this.token,
      this.groupKey,
      this.groupID,
      properties
  ...
  unset(prop) {
    ...
    this.oursprivacyImpl.groupUnsetProperty(
      this.token,
      this.groupKey,
      this.groupID,
      prop
  ...
  remove(name, value) {
    ...
    this.oursprivacyImpl.groupRemovePropertyValue(
      this.token,
      this.groupKey,
      this.groupID,
      name,
  ...
  union(name, value) {
    ...
    value = Array.isArray(value) ? value : [value];
    this.oursprivacyImpl.groupUnionProperty(
      this.token,
      this.groupKey,
      this.groupID,
      name,
