...
public class AutomaticEventsTest {
    ...
    private OursPrivacyAPI mCleanOursPrivacyAPI;
    ...
    private OPDbAdapter mockAdapter;
    ...
    private void setUpInstance(boolean trackAutomaticEvents) {
        ...
        InstrumentationRegistry.getInstrumentation().getContext().deleteDatabase("oursprivacy");
        mockAdapter = new OPDbAdapter(InstrumentationRegistry.getInstrumentation().getContext(), OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null)) {
            @Override
        ...
        final AnalyticsMessages automaticAnalyticsMessages = new AnalyticsMessages(InstrumentationRegistry.getInstrumentation().getContext(), OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null)) {
        ...
        mCleanOursPrivacyAPI = new OursPrivacyAPI(InstrumentationRegistry.getInstrumentation().getContext(), mMockReferrerPreferences, TOKEN, false, null, trackAutomaticEvents) {
    ...
    public void testAutomaticOneInstance() throws InterruptedException {
        ...
        setUpInstance(true);
        mCleanOursPrivacyAPI.track("An event One");
    ...
    public void testDisableAutomaticEvents() throws InterruptedException {
        ...
        setUpInstance(false);
        mLatch = new CountDownLatch(calls);
        mCleanOursPrivacyAPI.track("An Event Three");
        assertTrue(mLatch.await(MAX_TIMEOUT_POLL, TimeUnit.MILLISECONDS));
        assertEquals(calls, mTrackedEvents);

        mCleanOursPrivacyAPI.track("Automatic Event Two", null, true); // dropped
        mCleanOursPrivacyAPI.track("Automatic Event Three", null, true); // dropped
        mCleanOursPrivacyAPI.track("Automatic Event Four", null, true); // dropped
        mCleanOursPrivacyAPI.flush();
    ...
    public void testAutomaticMultipleInstances() throws InterruptedException {
        ...
        final OPDbAdapter mpSecondDbAdapter = new OPDbAdapter(InstrumentationRegistry.getInstrumentation().getContext(), OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null)) {
            @Override
            public void cleanupEvents(String last_id, Table table, String token) {
        ...
        final AnalyticsMessages mpSecondAnalyticsMessages = new AnalyticsMessages(InstrumentationRegistry.getInstrumentation().getContext(), OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null)) {
            @Override
            protected RemoteService getPoster() {
        ...
        OursPrivacyAPI mpSecondInstance = new TestUtils.CleanOursPrivacyAPI(InstrumentationRegistry.getInstrumentation().getContext(), new TestUtils.EmptyPreferences(InstrumentationRegistry.getInstrumentation().getContext()), SECOND_TOKEN, false) {
            @Override
            AnalyticsMessages getAnalyticsMessages() {
        ...
        mLatch = new CountDownLatch(OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null).getBulkUploadLimit() - initialCalls);
        for (int i = 0; i < OPConfig.getInstance(InstrumentationRegistry.getInstrumentation().getContext(), null).getBulkUploadLimit() - initialCalls; i++) {
            mCleanOursPrivacyAPI.track("Track event " + i);
        }
        ...
        mCleanOursPrivacyAPI.track("First Instance Event One");
