...
/**
 * SQLite database adapter for OursPrivacyAPI.
 *
 * <p>Not thread-safe. Instances of this class should only be used
 * by a single thread.
 *
 */
/* package */ class OPDbAdapter {
    private static final String LOGTAG = "OursPrivacyAPI.Database";
    private static final Map<String, OPDbAdapter> sInstances = new HashMap<>();
    ...
    private static final String DATABASE_NAME = "oursprivacy";
    ...
    private static class MPDatabaseHelper extends SQLiteOpenHelper {
        MPDatabaseHelper(Context context, String dbName, OPConfig config) {
        ...
        public void onCreate(SQLiteDatabase db) {
            OPLog.v(LOGTAG, "Creating a new OursPrivacy events DB");
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            OPLog.v(LOGTAG, "Upgrading app, replacing OursPrivacy events DB");
            ...
            if (prefsDir.exists() && prefsDir.isDirectory()) {
                String[] storedPrefsFiles = prefsDir.list(new FilenameFilter() {
                    ...
                    public boolean accept(File dir, String name) {
                        return name.startsWith("com.oursprivacy.android.opmetrics.OursPrivacyAPI_");
                    }
                });
    ...
    public OPDbAdapter(Context context, OPConfig config) {
    ...
    public OPDbAdapter(Context context, String dbName, OPConfig config) {
        mDb = new MPDatabaseHelper(context, dbName, config);
    }

    public static OPDbAdapter getInstance(Context context, OPConfig config) {
        synchronized (sInstances) {
            final Context appContext = context.getApplicationContext();
            OPDbAdapter ret;
            String instanceName = config.getInstanceName();
            if (!sInstances.containsKey(instanceName)) {
                ret = new OPDbAdapter(appContext, config);
    ...
    public int addJSON(JSONObject j, String token, Table table) {
        ...
        if (this.aboveMemThreshold()) {
            OPLog.e(LOGTAG, "There is not enough space left on the device or " +
        ...
        try {
            ...
            OPLog.e(LOGTAG, "Could not add OursPrivacy data to table");
            ...
            OPLog.e(LOGTAG, "Out of memory when adding OursPrivacy data to table");
        } finally {
            if (c != null) {
                c.close();
            }
    ...
    /* package */ int rewriteEventDataWithProperties(Map<String, String> properties, String token) {
        ...
        try {
            ...
            try {
                while (selectCursor.moveToNext()) {
                    try {
                        ...
                        JSONObject existingProps = updatedData.getJSONObject("event_properties");
                        for (final Map.Entry<String, String> entry : properties.entrySet()) {
                            final String key = entry.getKey();
                            final String value = entry.getValue();
                            existingProps.put(key, value);
                        }
                        updatedData.put("event_properties", existingProps);
            ...
            OPLog.e(LOGTAG, "Could not re-write events history. Re-initializing database.", e);
    ...
    public void cleanupEvents(String last_id, Table table, String token) {
        ...
        try {
            ...
            OPLog.e(LOGTAG, "Could not clean sent OursPrivacy records from " + tableName + ". Re-initializing database.", e);
            ...
            OPLog.e(LOGTAG, "Unknown exception. Could not clean sent OursPrivacy records from " + tableName + ".Re-initializing database.", e);
            mDb.deleteDatabase();
        } finally {
            mDb.close();
        }
    ...
    public void cleanupEvents(long time, Table table) {
        ...
        try {
            ...
            OPLog.e(LOGTAG, "Could not clean timed-out OursPrivacy records from " + tableName + ". Re-initializing database.", e);
    ...
    /**
     * Returns the data string to send to OursPrivacy and the maximum ID of the row that
